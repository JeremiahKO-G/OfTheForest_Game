...
void KeyboardInput::get_input() {
    ...
    if (key_states[SDL_SCANCODE_A] && key_states[SDL_SCANCODE_LSHIFT]) {
        next_action_type = ActionType::SprintLeft;
    }
    if (key_states[SDL_SCANCODE_D] && key_states[SDL_SCANCODE_LSHIFT]) {
        next_action_type = ActionType::SprintRight;
    }
    if (key_states[SDL_SCANCODE_A] && key_states[SDL_SCANCODE_S]) {
        next_action_type = ActionType::DodgeLeft;
    }
    if (key_states[SDL_SCANCODE_D] && key_states[SDL_SCANCODE_S]) {
        next_action_type = ActionType::DodgeRight;
    }
}
