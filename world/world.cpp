...
void World::update(float dt) {
    for (auto& obj : game_objects) {
        ...
        // check for collision with projectiles and enemy
        for (auto& projectile : projectiles) {
...
void World::update_object(GameObject* obj, double dt) {
    float terminal_velocity = obj->physics.terminal_velocity;
    obj->update(*this, dt);
    auto velocity = obj->physics.velocity;
    auto position = obj->physics.position;

    velocity += 0.5f * obj->physics.acceleration * static_cast<float>(dt);
...
void World::load_level(const Level& level) {
    ...
    for (const auto& [pos, enemy_name] : level.enemy_locations) {
        auto enemy = new GameObject{enemy_name, nullptr, nullptr, {255, 255, 0, 0}};
